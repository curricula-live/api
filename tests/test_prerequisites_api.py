...
def prerequisite_graph(db):
    ...
    connect("c", "a")
    connect("d", "c")
    connect("target", "d")  # cycle back to the traversal root
...
def test_prerequisites_traverses_incoming_edges_breadth_first(client, prerequisite_graph):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]), {"max_depth": 3}
    )
...
def test_prerequisites_respects_depth_bound(client, prerequisite_graph):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]), {"max_depth": 1}
    )
...
def test_prerequisites_respects_node_bound_and_reports_truncation(
    client, prerequisite_graph
):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]), {"max_nodes": 1}
    )
...
def test_prerequisites_handles_cycles_without_repeating_root(client, prerequisite_graph):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]), {"max_depth": 10}
    )
...
def test_prerequisites_rejects_invalid_bounds(client, prerequisite_graph, parameter, value):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]), {parameter: value}
    )

    assert response.status_code == 400


def test_prerequisites_accepts_zero_bounds(client, prerequisite_graph):
    response = client.get(
        reverse("concept-prerequisites", args=["target"]),
